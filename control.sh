...
if [[ "$mode" == "run" ]]; then
    ...
    run_tiniba.sh -r run -k 40 -N half-slab -x 2 -P 200 -c
    ...
    rm -rf .ab_layers.d .acell.d .back.layers back.layers.xy .cores eigen_* .fnval .front.layers front.layers.xy half_slab.* hoy* .i* layers.d* .lista_layers logfiles/ .machines_pmn .machines_scf .machines_latm .machines_res me_* mtita opt.dat .peso* si_as_6_* *.klist* si_as_6.xyz.original spin_info symmetries/ tmp* .trueMachines
fi
